      ...
      .then(data => {
        if (data.items && data.items.length > 0) {
          const dbPrograms: Program[] = data.items.map((item: any) => ({
            ...
            fields: Array.isArray(item.fields) ? item.fields : (typeof item.fields === 'string' ? item.fields.split(',').map(s => s.trim()) : ["All"]),
            format: (item.studyMode as "Online" | "Offline" | "Hybrid") || "Online",
            duration: item.duration || "Varies",
            tuition: item.tuition || 0,
            tags: Array.isArray(item.tags) ? item.tags : (typeof item.tags === 'string' ? item.tags.split(',').map(s => s.trim()) : ["General"]),
