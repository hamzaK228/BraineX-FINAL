...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logAdminActivity({ adminId: session.user!.id!, action: "UPDATE", target: "field", targetId: id, details: `Updated field: ${item.title}` });
  return NextResponse.json(item);
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const { id } = await params;
  const item = await prisma.contentField.findUnique({ where: { id }, select: { title: true } });
  await prisma.contentField.delete({ where: { id } });
  await logAdminActivity({ adminId: session.user!.id!, action: "DELETE", target: "field", targetId: id, details: `Deleted field: ${item?.title || id}` });
