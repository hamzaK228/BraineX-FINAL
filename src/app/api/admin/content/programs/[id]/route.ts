...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logAdminActivity({ adminId: session.user!.id!, action: "UPDATE", target: "program", targetId: id, details: `Updated program: ${item.title}` });
  return NextResponse.json(item);
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const { id } = await params;
  const item = await prisma.contentProgram.findUnique({ where: { id }, select: { title: true } });
  await prisma.contentProgram.delete({ where: { id } });
  await logAdminActivity({ adminId: session.user!.id!, action: "DELETE", target: "program", targetId: id, details: `Deleted program: ${item?.title || id}` });
